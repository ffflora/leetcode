### 1204, Last Person to Fit in the Bus, SQL, Medium
...
SELECT person_name  FROM (
    SELECT *, sum(weight) OVER(ORDER BY turn) as wt
    FROM Queue 
    ORDER BY wt DESC
) as t
WHERE wt <=1000
...
- Accumulative sum: `SUM(٩(ü)ว) OVER( ORDER BY ...)` in which `ORDER BY` makes it accumulative
