### 1532. The Most Recent Three Orders, SQL, Medium
...
SELECT C.name customer_name, A.customer_id, A.order_id, A.order_date 
FROM (
...
WHERE A.rk <= 3
ORDER BY customer_name, C.customer_id, A.order_date DESC

```

### Notes:

- get the rank for each person first, then left join the ranking result with the Customer table. 
