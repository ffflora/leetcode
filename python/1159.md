#### [1159. Market Analysis II](https://leetcode.com/problems/market-analysis-ii/), SQL - Hard
...
SELECT seller_id, (CASE
    WHEN favorite_brand = sec_name THEN 'yes'
    ELSE 'no'
    END) 2nd_item_fav_brand 
FROM(
    SELECT U.user_id seller_id, U.favorite_brand,  withrk.item_id second_id, I.item_brand sec_name 
    FROM Users U
    LEFT JOIN
    ...
    ON U.user_id = withrk.seller_id 
    LEFT JOIN Items I
    ON withrk.item_id = I.item_id 
    ) t2
...
1. The order of joining the tables matters. 
